import { useSelector, useDispatch } from "react-redux";
const EventCard = ({ setEvent }) => {
    const styleMode = useSelector((state) => state.styleModer.mode);
    return (
        <div className={`${styleMode ? "bg-black " : "bg-white"}   max-w-xs mx-auto xl:px-3 lg:px-1 mt-2 `}>
            <div className={` ${styleMode ? "border-[#6B6B6B] border bg-black" : "bg-white border border-light_border"}  rounded-2xl `}>
                <div className="relative">
                    <div className="absolute right-2 top-4">
                        <div className={` ${styleMode ? "border-white" : "border-white"}  flex items-center border  rounded-3xl `}>
                            <span className="bg-black h-7 px-3 flex items-center rounded-l-3xl">
                                <svg xmlns="http://www.w3.org/2000/svg" width="21" height="21" viewBox="0 0 21 21" fill="none">
                                    <path opacity="0.35" d="M12.9594 1.91797C12.1244 1.91797 11.4102 2.41214 11.0777 3.12047C10.7927 2.89297 10.4361 2.7513 10.0427 2.7513C9.64939 2.7513 9.29272 2.89297 9.00772 3.12047C8.67522 2.41214 7.96106 1.91797 7.12606 1.91797C6.29106 1.91797 5.57689 2.41214 5.24439 3.12047C4.95939 2.89297 4.60272 2.7513 4.20939 2.7513C3.28856 2.7513 2.54272 3.49714 2.54272 4.41797C2.54272 5.3388 3.28856 6.08464 4.20939 6.08464C4.77272 6.08464 5.26939 5.80297 5.57106 5.37464C5.80522 5.63964 6.10272 5.8413 6.44356 5.96047C6.10189 6.67463 5.87606 7.72964 5.87606 8.16797C5.87606 8.85797 6.43606 9.41797 7.12606 9.41797C7.81606 9.41797 8.37606 8.85797 8.37606 8.16797C8.37606 7.72964 8.15022 6.67463 7.80939 5.96047C8.15022 5.8413 8.44772 5.63964 8.68189 5.37464C8.98272 5.80297 9.47939 6.08464 10.0427 6.08464C10.6061 6.08464 11.1027 5.80297 11.4044 5.37464C11.7861 5.80714 12.3377 6.08464 12.9594 6.08464C14.1102 6.08464 15.0427 5.15214 15.0427 4.0013C15.0427 2.85047 14.1102 1.91797 12.9594 1.91797Z" fill="white" />
                                    <path d="M15.4594 6.91736C15.1694 6.91736 14.891 6.95736 14.6202 7.01986L14.4444 5.45986C14.066 5.84486 13.541 6.08402 12.9594 6.08402C12.3377 6.08402 11.786 5.80652 11.4044 5.37402C11.1027 5.80236 10.606 6.08402 10.0427 6.08402C9.47936 6.08402 8.9827 5.80236 8.68103 5.37402C8.4477 5.63902 8.14936 5.84152 7.80936 5.95986C8.1502 6.67402 8.37603 7.72902 8.37603 8.16736C8.37603 8.85736 7.81603 9.41736 7.12603 9.41736C6.43603 9.41736 5.87603 8.85736 5.87603 8.16736C5.87603 7.72902 6.10186 6.67402 6.4427 5.95986C6.1027 5.84152 5.80436 5.63902 5.57103 5.37402C5.26936 5.80236 4.7727 6.08402 4.20936 6.08402C4.10353 6.08402 4.00103 6.07152 3.90103 6.05319L3.01103 14.004C2.7627 15.9932 4.31353 17.7507 6.31853 17.7507H12.1002C13.9652 17.7507 15.4177 16.2265 15.4144 14.4132C15.4302 14.4132 15.4444 14.4174 15.4594 14.4174C17.5269 14.4174 19.2094 12.7349 19.2094 10.6674C19.2094 8.59986 17.5269 6.91736 15.4594 6.91736ZM15.4594 11.9174C15.3585 11.9174 15.2619 11.9024 15.1677 11.8799L14.9052 9.55236C15.0727 9.46902 15.2594 9.41736 15.4594 9.41736C16.1494 9.41736 16.7094 9.97736 16.7094 10.6674C16.7094 11.3574 16.1494 11.9174 15.4594 11.9174Z" fill="white" />
                                </svg>
                            </span>
                            <span className="bg-bg_dark h-7 px-3 text-xs font-plus text-white flex items-center rounded-r-3xl" >
                                Pubs & Bars
                            </span>
                        </div>
                    </div>
                    <img src="/img/wedding-1-1.jpg" className="rounded-t-2xl" alt="img" />
                </div>
                <div className="px-2 pt-2 pb-3 relative">
                    <h4 className={`font-bold font-plus ${styleMode ? "text-white" : ""} `}>Karaoke Evening</h4>
                    <ul className={` ${styleMode ? "text-[#BCBCBC]" : "text-light_text"} py-3 flex item-center gap-2 `}>
                        <li className="flex items-center">
                            <span><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                                <path opacity="0.35" d="M7.94773 13.9609C11.2399 13.9609 13.9086 11.2921 13.9086 7.99997C13.9086 4.70785 11.2399 2.03906 7.94773 2.03906C4.65561 2.03906 1.98682 4.70785 1.98682 7.99997C1.98682 11.2921 4.65561 13.9609 7.94773 13.9609Z" fill={` ${styleMode ? "white" : "black"}`} stroke="black" stroke-width="1.58958" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M10.0016 9.45631C10.0016 7.20441 7.48477 7.53557 7.48477 6.34339C7.48477 5.68107 8.01462 5.5486 8.21332 5.5486C8.41202 5.5486 8.54448 5.61483 8.67695 5.68107C9.00811 5.81353 9.4055 5.68107 9.6042 5.41614C9.86913 5.01874 9.73667 4.48888 9.33927 4.29019C9.14057 4.15772 8.80941 4.09149 8.47825 4.09149V3.76033C8.47825 3.42917 8.21332 3.23047 7.94839 3.23047C7.68346 3.23047 7.41853 3.4954 7.41853 3.76033V4.22395C6.49128 4.48888 5.89519 5.3499 5.89519 6.40962C5.89519 8.79399 8.34579 8.33036 8.34579 9.58877C8.34579 9.8537 8.21332 10.3173 7.61723 10.3173C7.3523 10.3173 7.08737 10.2511 6.95491 10.1186C6.62374 9.98617 6.22635 10.1186 6.02765 10.3836C5.76272 10.7147 5.89519 11.2446 6.22635 11.4433C6.55751 11.642 6.88867 11.7744 7.3523 11.7744V12.1718C7.3523 12.503 7.61723 12.7017 7.88216 12.7017C8.21332 12.7017 8.41202 12.4368 8.41202 12.1718V11.7082C9.47174 11.4433 10.0016 10.516 10.0016 9.45631Z" fill={` ${styleMode ? "white" : "black"}`} />
                            </svg></span>
                            <span className=" text-xs font-plus">
                                Free
                            </span>
                        </li>
                        <li className="flex items-center">
                            <span><svg xmlns="http://www.w3.org/2000/svg" width="17" height="16" viewBox="0 0 17 16" fill="none">
                                <path opacity="0.35" d="M8.1496 14.6224C11.8075 14.6224 14.7728 11.6571 14.7728 7.99921C14.7728 4.3413 11.8075 1.37598 8.1496 1.37598C4.49169 1.37598 1.52637 4.3413 1.52637 7.99921C1.52637 11.6571 4.49169 14.6224 8.1496 14.6224Z" fill={` ${styleMode ? "white" : "black"}`} />
                                <path d="M8.81182 3.95975L9.14298 7.66876C9.20921 8.19862 8.81182 8.66225 8.21572 8.72848C7.61963 8.79471 7.22224 8.39732 7.15601 7.80123C7.15601 7.735 7.15601 7.66876 7.15601 7.60253L7.48717 3.95975C7.48717 3.56236 7.81833 3.29743 8.21572 3.36366C8.54689 3.42989 8.81182 3.69482 8.81182 3.95975Z" fill={` ${styleMode ? "white" : "black"}`} />
                                <path d="M9.34203 10.1193L7.48752 8.52977C7.09012 8.1986 7.02389 7.53628 7.42129 7.13889C7.75245 6.74149 8.41477 6.67526 8.81217 7.07265C8.8784 7.13889 8.94463 7.20512 8.94463 7.27135L10.4017 9.32455C10.6004 9.65572 10.5342 10.0531 10.2693 10.2518C9.93812 10.3843 9.60696 10.318 9.34203 10.1193Z" fill={` ${styleMode ? "white" : "black"}`} />
                            </svg></span>
                            <span className=" text-xs font-plus">
                                7:45-9:30
                            </span>
                        </li>
                        <li className="flex items-center">
                            <span><svg xmlns="http://www.w3.org/2000/svg" width="17" height="16" viewBox="0 0 17 16" fill="none">
                                <path opacity="0.35" d="M5.3716 6.67533C6.65187 6.67533 7.68973 5.63746 7.68973 4.35719C7.68973 3.07693 6.65187 2.03906 5.3716 2.03906C4.09133 2.03906 3.05347 3.07693 3.05347 4.35719C3.05347 5.63746 4.09133 6.67533 5.3716 6.67533Z" fill={` ${styleMode ? "white" : "black"}`} />
                                <path opacity="0.35" d="M11.3325 6.67533C12.6128 6.67533 13.6507 5.63746 13.6507 4.35719C13.6507 3.07693 12.6128 2.03906 11.3325 2.03906C10.0523 2.03906 9.0144 3.07693 9.0144 4.35719C9.0144 5.63746 10.0523 6.67533 11.3325 6.67533Z" fill={` ${styleMode ? "white" : "black"}`} />
                                <path opacity="0.35" d="M7.3584 9.32422H12.657C13.9353 9.32422 14.9751 10.3641 14.9751 11.6424C14.9751 12.9206 13.9353 13.9605 12.657 13.9605H7.3584V9.32422Z" fill={` ${styleMode ? "white" : "black"}`} />
                                <path d="M9.67615 11.6424C9.67615 12.9206 8.63631 13.9605 7.35802 13.9605H4.0464C3.41057 13.9605 2.82773 13.7022 2.41046 13.2783C1.98658 12.861 1.72827 12.2782 1.72827 11.6424C1.72827 10.3641 2.76812 9.32422 4.0464 9.32422H7.35802C7.99385 9.32422 8.5767 9.58252 8.99396 10.0064C9.41785 10.4237 9.67615 11.0065 9.67615 11.6424Z" fill={` ${styleMode ? "white" : "black"}`} />
                            </svg></span>
                            <span className=" text-xs font-plus">
                                12 guests
                            </span>
                        </li>
                    </ul>
                    <div className="flex items-center">
                        <p className={` ${styleMode ? "text-[#808080]" : "text-[#BCBCBC]"} text-sm font-plus `}>Starts in </p>
                        <span className="px-1"><svg xmlns="http://www.w3.org/2000/svg" width="17" height="16" viewBox="0 0 17 16" fill="none">
                            <path opacity="0.35" d="M8.1496 14.6224C11.8075 14.6224 14.7728 11.6571 14.7728 7.99921C14.7728 4.3413 11.8075 1.37598 8.1496 1.37598C4.49169 1.37598 1.52637 4.3413 1.52637 7.99921C1.52637 11.6571 4.49169 14.6224 8.1496 14.6224Z" fill={` ${styleMode ? "white" : "black"}`} />
                            <path d="M8.81182 3.95975L9.14298 7.66876C9.20921 8.19862 8.81182 8.66225 8.21572 8.72848C7.61963 8.79471 7.22224 8.39732 7.15601 7.80123C7.15601 7.735 7.15601 7.66876 7.15601 7.60253L7.48717 3.95975C7.48717 3.56236 7.81833 3.29743 8.21572 3.36366C8.54689 3.42989 8.81182 3.69482 8.81182 3.95975Z" fill={` ${styleMode ? "white" : "black"}`} />
                            <path d="M9.34203 10.1193L7.48752 8.52977C7.09012 8.1986 7.02389 7.53628 7.42129 7.13889C7.75245 6.74149 8.41477 6.67526 8.81217 7.07265C8.8784 7.13889 8.94463 7.20512 8.94463 7.27135L10.4017 9.32455C10.6004 9.65572 10.5342 10.0531 10.2693 10.2518C9.93812 10.3843 9.60696 10.318 9.34203 10.1193Z" fill={` ${styleMode ? "white" : "black"}`} />
                        </svg></span>
                        <span className="text-sm font-plus font-bold text-theme_blue">3 Days</span>
                    </div>
                    <div className={` ${styleMode ? "bg-[#6B6B6B]" : "bg-light_border "} absolute right-4 bottom-3  rounded-full cursor-pointer `}>
                        <div className="relative w-5 h-5 " onClick={() => setEvent(true)}>
                            <svg className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 17 17" fill="none">
                                <path opacity="0.35" d="M10.3009 9.80145C9.91278 9.41333 6.05276 5.55331 5.66464 5.16519C4.88839 4.38895 3.63064 4.38895 2.8544 5.16519C2.07816 5.94143 2.07816 7.19919 2.8544 7.97543C3.24252 8.36355 7.10254 12.2236 7.49066 12.6117C8.26691 13.3879 9.52466 13.3879 10.3009 12.6117C11.0771 11.8354 11.0771 10.577 10.3009 9.80145Z" fill={` ${styleMode ? "white" : "black"}`} />
                                <path d="M10.3011 12.6107C10.6892 12.2226 14.5493 8.36257 14.9374 7.97445C15.7136 7.19821 15.7136 5.94046 14.9374 5.16421C14.1611 4.38797 12.9034 4.38797 12.1271 5.16421C11.739 5.55234 7.879 9.41236 7.49087 9.80048C6.71463 10.5767 6.71463 11.8345 7.49087 12.6107C8.26712 13.387 9.52553 13.387 10.3011 12.6107Z" fill={` ${styleMode ? "white" : "black"}`} />
                            </svg>
                        </div>

                    </div>
                </div>
            </div>
        </div>
    )
}
export default EventCard;